...
const resolveConfig = (
  ...
  const mapped = isHttpError ? HTTP_ERROR_MAP[errorCode ?? 0] : BUSINESS_ERROR_MAP[errorCode ?? 0]
  return {
...
export const handleApiError = (error: AxiosError<ApiErrorBody>): void => {
  ...
  if (!error.response) {
    toast.error("Lỗi kết nối mạng. Vui lòng kiểm tra internet.", { duration: 4000 })
    return
  }

  const errorCode = isHttpError ? httpStatus : businessErrorCode

  const { message, tier } = resolveConfig(serverMessage, errorCode, isHttpError)
