...
export const CustomerMenuClient = ({
  ...
  // const occupiedModalShown = useRef(false);

  useEffect(() => {
    const hasShownModal = sessionStorage.getItem(`occupied_modal_${tableName}`);
    if (isOccupied && !hasShownModal) {
      sessionStorage.setItem(`occupied_modal_${tableName}`, "true");
        ...
        <header className="sticky top-0 z-10 bg-white border-b border-gray-100 shadow-sm">
          <div className="px-4 py-3 flex items-center justify-between gap-3">
            ...
            <button
              ...
              className="text-xs font-medium text-brand-600 dark:text-brand-400 bg-brand-50 dark:bg-brand-900/20 rounded-full px-3 py-1.5 hover:bg-brand-100 dark:hover:bg-brand-900/30 transition-colors"
            >
              📋 Đơn của tôi
            </button>
          </div>
