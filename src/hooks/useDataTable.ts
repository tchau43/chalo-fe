...
export interface UseDataTableProps<T, F extends BaseFilter> {
  queryKey: string
...
export interface UseDataTableReturn<T, F extends BaseFilter> {
  data: T[],
  total: number,
  isLoading: boolean,
  isFetching: boolean,
...
export function useDataTable<T, F extends BaseFilter>({ queryKey, queryFn, initialFilter, staleTime = 30_000 }: UseDataTableProps<T, F>): UseDataTableReturn<T, F> {
  const [filter, setFilter] = useState<F>(initialFilter)
  const qc = useQueryClient();

  const qkFull = useMemo(() => {
    return [queryKey, 'page', filter]
  }, [queryKey, filter])
  ...
  const updateFilter = useCallback((params: Partial<Omit<F, 'pageNo' | 'pageSize'>>) => {
  ...
  const refresh = useCallback(() => {
    qc.invalidateQueries({ queryKey: [queryKey, 'page'] })
  }, [queryKey, qc])
